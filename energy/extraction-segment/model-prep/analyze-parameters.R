...
para_year = fread(file.path(yr_file))

prod_2019 = fread(file.path(prod_file), header = T)
prod_2019 = prod_2019[year == 2019]

df_entry = fread(file.path(entry_file), header = T)
...
fwrite(pred_all, file.path(save_path, save_file), row.names = F)
