...
# source_path   <- paste0(main_path, 'data-str/public/health/source_receptor_matrix/')
# inmap_ex_path  <- paste0(main_path, "data-str/public/health/source_receptor_matrix/inmap_processed_srm/extraction")
# inmap_ref_path  <- paste0(main_path, "data/health/source_receptor_matrix/inmap_processed_srm/refining")


## update these as needed
save_info_path <- paste0(main_path, 'data-str/public/outputs/model-out/extraction/figures/revision-replicate/')
...
## well prod
well_prod <- fread("/capstone/freshcair/meds-freshcair-capstone/data-str/public/outputs/results-out/well_prod_m_processed.csv", colClasses = c('api_ten_digit' = 'character',
                                                                                                 'doc_field_code' = 'character'))

## read in bau field out
bau_out <- readRDS(paste0('/capstone/freshcair/meds-freshcair-capstone/data-str/public/outputs/model-out/', bau_file))

## county out
county_out <- readRDS(paste0('/capstone/freshcair/meds-freshcair-capstone/data-str/public/outputs/model-out/', labor_file))

## load opex/ capex
price_data = fread(file.path('/capstone/freshcair/meds-freshcair-capstone/data-str/public/intermediate/energy/production/field_capex_opex_forecast_revised.csv'), header = T)
...
## emissions factors
ghg_factors = fread(file.path('/capstone/freshcair/meds-freshcair-capstone/data-str/public/intermediate/energy/production/', ghg_file), header = T)
...
## setback coverage
setback_scens = fread(file.path('/capstone/freshcair/meds-freshcair-capstone/data-str/private/setback-buffs', setback_file), header = T, colClasses = c('doc_field_code' = 'character'))
...
## dac 
dac_ces <- read_xlsx('/capstone/freshcair/meds-freshcair-capstone/data-str/public/inputs/health/ces3results.xlsx')
...
## census pop
ct_inc_pop_45 <- fread("/capstone/freshcair/meds-freshcair-capstone/data-str/public/outputs/health-out/ct_inc_45.csv", stringsAsFactors  = FALSE) %>%
...
## save 
fwrite(field_df, paste0('/capstone/freshcair/meds-freshcair-capstone/data-str/public/outputs/results-out/field_characteristics.csv'))

fwrite(county_df, paste0('/capstone/freshcair/meds-freshcair-capstone/data-str/public/outputs/results-out/county_characteristics.csv'))
