...
# Added MP
setback_2025_df <- state_levels[((policy_intervention == "BAU" & year >= 2020 & year <= 2024) |
                                   (setback_scenario == "setback_3200ft" & year >= 2025 & year <= 2045)), ]
...
# Added MP
scc_value_2025 <- setback_2025_df[metric == 'total_state_ghg_MtCO2']
...
# Added MP
bau_2025_df <- state_levels[policy_intervention == "BAU" & year >= 2025 & year <= 2045]

setback_2025_df <- merge(setback_2025_df, bau_2025_df[, .(scen_id, oil_price_scenario, setback_existing, innovation_scenario, carbon_price_scenario, ccs_scenario, setback_scenario, excise_tax_scenario, target, target_policy, metric, year, value)],
                         by = c("scen_id", "oil_price_scenario", "setback_existing", "innovation_scenario", "carbon_price_scenario", "ccs_scenario", "setback_scenario", "excise_tax_scenario", "target", "target_policy", "metric", "year"),
                         suffixes = c("", "_bau"))
setback_2025_df[, diff_bau := value - value_bau]
setback_2025_df[, value_bau := NULL]

scc_value_2025 <- setback_2025_df[metric == 'total_state_ghg_MtCO2']
scc_value_2025 <- scc_value_2025[, .(scen_id, oil_price_scenario, setback_existing, innovation_scenario, carbon_price_scenario, ccs_scenario, setback_scenario, excise_tax_scenario, target, target_policy, year, metric, value, policy_intervention, diff_bau)]

## deflate to 2019 dollars
scc_value[, social_cost_usd19 := social_cost_co2 * (cpi2019 / cpi2020)]

# Added MP
scc_value_2025[, social_cost_usd19 := social_cost_co2 * (cpi2019 / cpi2020)]

scc_value[, scc_avoided_ghg := diff_bau * -1e6 * social_cost_usd19]

# Added MP
scc_value_2025[, scc_avoided_ghg := diff_bau * -1e6 * social_cost_usd19]

## apply discount rate
scc_value <- scc_value[year > 2019]

# Added MP
scc_value_2025 <- scc_value_2025[year >= 2025]

scc_value[, scc_avoided_ghg_pv := scc_avoided_ghg / ((1 + discount_rate) ^ (year - 2019))]

# Added MP
scc_value_2025[, scc_avoided_ghg_pv := scc_avoided_ghg / ((1 + discount_rate) ^ (year - 2025))]
...
# Added MP
cumul_scc_value_2025 <- scc_value_2025[, .(scc_avoided_ghg = sum(scc_avoided_ghg, na.rm = T),
                                           scc_avoided_ghg_pv = sum(scc_avoided_ghg_pv)), by = .(scen_id, oil_price_scenario, setback_existing,
                                                                                                 policy_intervention, target)]
...
# Added MP
cumul_rel_vals_bau_2025 <- setback_2025_df[, .(diff_bau = sum(diff_bau)), by = .(scen_id, oil_price_scenario, setback_existing, policy_intervention,
                                                                                 target, metric)]
...
# Added MP
cumul_rel_vals_bau_2025 <- cumul_rel_vals_bau_2025[metric %in% c("total_state_ghg_MtCO2", "total_comp_usd19", "total_comp_PV", "cost_2019",
                                                                 "cost", "cost_2019_PV", "cost_PV")]
...
# Added MP
cumul_rel_vals_bau_2025 <- dcast(cumul_rel_vals_bau_2025, scen_id + oil_price_scenario + setback_existing + policy_intervention + target ~ metric, value.var = "diff_bau")
...
# Added MP
cumul_rel_vals_bau_2025 <- merge(cumul_rel_vals_bau_2025, cumul_scc_value_2025,
                                 by = c('scen_id', 'oil_price_scenario', 'setback_existing', 'policy_intervention', 'target'),
                                 all.x = T)
...
# Added MP
ghg_total_2025 <- setback_2025_df[metric == "total_state_ghg_MtCO2"]
ghg_total_2025 <- ghg_total_2025[, .(cumul_ghg = sum(value)), by = .(scen_id)]
...
# Added MP
cumul_rel_vals_bau_2025 <- merge(cumul_rel_vals_bau_2025, ghg_total_2025,
                                 by = "scen_id",
                                 all.x = T)
...
# Added MP
cumul_rel_vals_bau_2025[, benefit := (cost_2019_PV * -1) + total_comp_PV + scc_avoided_ghg_pv]
cumul_rel_vals_bau_2025[, benefit_per_ghg := benefit / (-1 * total_state_ghg_MtCO2)]
cumul_rel_vals_bau_2025[, benefit_per_ghg := fifelse(is.na(benefit_per_ghg), 0, benefit)]
...
npv_x_metric_2025 <- melt(cumul_rel_vals_bau_2025, id.vars = c('scen_id', 'oil_price_scenario', 'setback_existing', 'policy_intervention', 'target', 'cumul_ghg', 'total_state_ghg_MtCO2'),
                          measure.vars = c("total_comp_PV", "cost_2019_PV", "scc_avoided_ghg_pv"),
                          variable.name = "metric",
                          value.name = "value")

npv_x_metric_2025[, value := fifelse(metric == 'cost_2019_PV', value * -1, value)]
npv_x_metric_2025[, value_per_ghg := value / (total_state_ghg_MtCO2 * -1)]
npv_x_metric_2025[, value_billion := value / 1e9]
npv_x_metric_2025[, value_per_ghg_million := value_per_ghg / 1e6]
npv_x_metric_2025[, title := fifelse(metric == "total_comp_PV", "Labor: Compensation",
                                     fifelse(metric == "cost_2019_PV", "Health: Avoided mortality", "Abated GHG"))]

npv_x_metric_2025[, value_per_ghg_million := fifelse(is.na(value_per_ghg_million), 0, value_per_ghg_million)]

npv_x_metric_2025 <- merge(npv_x_metric_2025, ghg_2045,
                           by = c('scen_id', 'oil_price_scenario', 'setback_existing'),
                           all.x = T)

## join with target label
npv_x_metric_2025 <- merge(npv_x_metric_2025, setback_2045,
                           by = c("oil_price_scenario", "target", 'setback_existing'),
                           all.x = T)

npv_x_metric_2025[, target_label := fifelse(policy_intervention == "BAU", target, 
                                            fifelse(target == "90perc_reduction", "90%", target_label))]

fwrite(npv_x_metric_2025, paste0('data/outputs/npv_x_metric_2025_oil.csv'))
