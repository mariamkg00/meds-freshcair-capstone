...
calc_2045_ghg <- function(inputs, scen) {
  ...
  for (i in seq_along(pred_years)) {
    ...
    exit_save = merge(dt_info_z[year == t, .(doc_field_code, doc_fieldname, oil_price_scenario,
                                             innovation_scenario, carbon_price_scenario, ccs_scenario, setback_scenario, setback_existing,  prod_quota_scenario, excise_tax_scenario)],
                      exit_save,
                      by = "doc_field_code",
                      all.x = T)
    ...
    # # ADDED MP START
    print("Column names of dt_info_rank before renaming:")
    
    if (!"cost_rank_cost_new" %in% colnames(dt_info_rank)) {
      dt_info_rank[, cost_rank_cost_new := NA]
    }
    if (!"cost_rank_cost_existing" %in% colnames(dt_info_rank)) {
      dt_info_rank[, cost_rank_cost_existing := NA]
    }
    
    # Rename columns (skip absent columns)
    setnames(dt_info_rank, 'cost_rank_cost_new', 'cost_new_rank', skip_absent = TRUE)
    setnames(dt_info_rank, 'cost_rank_cost_existing', 'cost_existing_rank', skip_absent = TRUE)
    # 
    # # ADDED MP END
