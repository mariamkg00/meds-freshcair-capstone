...
# load field-level production data -- Updated - MP
field_prod = fread(file.path("data/processed/crude_prod_x_field_revised.csv"), header = T)

# load field-level emissions factors -- Updated - MP
field_emfac = fread(file.path("data/intermediate-zenodo/intermediate/extraction-model/ghg_emissions_x_field_2018-2045.csv"), header = T)
# field_emfac = field_emfac[, .(doc_field_code, doc_fieldname, upstream_kgCO2e_bbl)]

# load refinery-level emissions -- Updated - MP but cheating cuz using Zenodo
ref_ghg = fread(file.path("data/intermediate-zenodo/intermediate/extraction-model/refinery_ghg_emissions.csv"), header = T)
