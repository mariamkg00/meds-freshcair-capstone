...
entry_df$test_predict_2 <- predict(train_rf_fit, entry_df)
...
wells_by_year <- new_wells_pred_revised %>%
  group_by(year) %>%
  summarise(new_wells_rf = sum(test_predict_2),
...
pred_actual_new_wells_yoy <- ggplot(wells_by_year, aes(x = year)) +
  geom_line(aes(y = new_wells_rf, color = "Random Forest"), size = 0.8, linetype = "solid") +
  geom_point(aes(y = new_wells_rf, color = "Random Forest"), size = 2) +
